...
categories = sorted(["Normal", "Pneumonia", "COVID"])
...
#report_dir = os.path.join("./reports", task, "covidx")
...
data_file = os.path.join("./data/processed", "covidx_normal_pneumonia_covid", "data.xlsx")
data = pd.read_excel(data_file, index_col=0, dtype=list)

# test file
test_file = os.path.join('./data/processed', "covidx_normal_pneumonia_covid", 'all.txt')
